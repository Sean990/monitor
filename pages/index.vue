    ...
    <view class="monitor-left">
      ...
      <view class="weather-box">
        <view class="info">
          <image
            class="icon"
            :src="`/static/icons/${weatherInfo.icon}.png`"
            mode=""
          ></image>
          <view class="temp"
            >{{ weatherInfo.temp }}℃ {{ weatherInfo.text }}</view
          >
        </view>
        <view class="other"
          >{{ cityName }} 湿度{{ weatherInfo.humidity }}%
          {{ weatherInfo.windDir }}{{ weatherInfo.windScale }}级</view
        >
      </view>
      ...
      <view class="server-box">
          ...
          <view class="progress">
            <view
              ...
              :style="`width: ${Math.round(
                (netWorkData.load.one / netWorkData.load.max) * 100
              )}%;`"
            ></view>
            <text class="occupy"
              >{{
                Math.round((netWorkData.load.one / netWorkData.load.max) * 100)
              }}%</text
            >
          </view>
        </view>
        ...
        <view class="bar-item">
          ...
          <view class="progress">
            <view
              class="active"
              :style="`width: ${(
                (netWorkData.mem.memRealUsed / netWorkData.mem.memTotal) *
                100
              ).toFixed(1)}%;`"
            ></view>
            <text class="occupy"
        ...
        <view class="bar-item">
          ...
          <view class="progress">
            <view
              class="active"
              :style="`width: ${netWorkData.disk[0].size[3]}`"
            ></view>
...
export default {
  ...
  methods: {
    appInit() {
      ...
      let WifiManager = plus.android.importClass(
